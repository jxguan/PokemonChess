		...
		jQuery(function($){ //on document.ready
			function snapback(dur){
				if ($selectedlink.length>0)
					$effectref.dequeue().animate({left:$selectedlink.position().left-1, width:$selectedlink.outerWidth() - 1}, dur, setting.fx)
			}
...
//alert($target.position().left+" "+$target.get(0).offsetLeft)
				...
				$effectref.dequeue().animate({left:$target.position().left-1, width:$target.outerWidth() - 1}, setting.fxtime, setting.fx)
